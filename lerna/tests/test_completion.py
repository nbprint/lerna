...
from lerna._internal.core_plugins.bash_completion import BashCompletion
from lerna._internal.utils import create_config_search_path
from lerna.plugins.completion_plugin import DefaultCompletionPlugin
...
def create_config_loader() -> ConfigLoaderImpl:
    return ConfigLoaderImpl(
        config_search_path=create_config_search_path(search_path_dir=os.path.realpath("lerna/test_utils/configs/completion_test"))
    )
...
def test_bash_completion_with_dot_in_path() -> None:
    process = subprocess.Popen(
        "lerna/tests/scripts/test_bash_dot_in_path.bash",
        shell=False,
...
def test_bash_completion_does_not_execute_non_hydra_script(tmp_path: Path) -> None:
    completion_app = Path("lerna/test_utils/completion.py").resolve()
    completion_test = Path("lerna/tests/scripts/test_bash_completion_non_hydra.bash").resolve()
    script = tmp_path / "non_hydra.py"
    script.write_text('from pathlib import Path\nPath("executed").touch()\n')

    subprocess.check_call(
        ["bash", str(completion_test), "python", str(completion_app)],
        cwd=tmp_path,
...
@mark.skipif(sys.platform == "win32", reason="does not run on windows")
def test_bash_completion_does_not_execute_python_option_as_script(tmp_path: Path) -> None:
    completion_app = Path("lerna/test_utils/completion.py").resolve()
    completion_test = Path("lerna/tests/scripts/test_bash_completion_non_hydra.bash").resolve()
    (tmp_path / "--").write_text("# @lerna.main\n")
    (tmp_path / "-sc").write_text('from pathlib import Path\nPath("executed").touch()\n')
...
@mark.parametrize(
    ...
    [
        ...
        param("group=dict +group=", 2, ["+group=dict", "+group=list"], id="group_append2"),
        ...
        param("group=dict ~group=", 2, ["~group=dict", "~group=list"], id="group_delete2"),
...
class TestRunCompletion:
    def test_completion_plugin(self, line_prefix: str, num_tabs: int, line: str, expected: list[str]) -> None:
        ...
        ret = bc._query(config_name="config.yaml", line="--multirun " + line_prefix + line)
    ...
    def test_shell_integration(
        ...
        if shell in ("zsh", "fish") and any(word.startswith("~") for word in line.split(" ")):
        ...
        assert os.path.realpath(ret.decode("utf-8").strip()) == os.path.realpath(sys.executable.strip())
        ...
        cmd.extend(
            [
                f"lerna/tests/scripts/test_{shell}_completion.exp",
                f"{' '.join(prog)}",
...
@mark.parametrize(
    ...
    [
        ...
        param("group=dict,list list.", ["list.0=", "list.1=", "list.2="], id="multirun"),
...
def test_strip(prefix: str, app_prefix: str, args_line: str, args_line_index: int) -> None:
...
@mark.parametrize(
    ...
    [
        (
            "bash",
            "lerna/tests/scripts/test_bash_install_uninstall.sh",
            "hydra_bash_completion",
        ),
        (
            "fish",
            "lerna/tests/scripts/test_fish_install_uninstall.fish",
...
def test_install_uninstall(shell: str, script: str, comp_func: str) -> None:
    if shell == "fish" and not is_fish_supported():
        skip("fish is not installed or the version is too old")
    cmd = [shell, script, "python lerna/test_utils/completion.py", comp_func]
