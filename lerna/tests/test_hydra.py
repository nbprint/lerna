...
def test_app_without_config__with_append(
    ...
    with hydra_task_runner(
        ...
        assert task.job_ret is not None and task.job_ret.cfg == {"abc": 123, "a": {"b": 1, "a": 2}}
...
def test_app_with_config_file__no_overrides(
    ...
    with task:
        assert task.job_ret is not None and task.job_ret.cfg == {"dataset": {"name": "imagenet", "path": "/datasets/imagenet"}}
...
def test_app_with_config_file__with_override(
    ...
    with hydra_task_runner(
        ...
        assert task.job_ret is not None and task.job_ret.cfg == {"dataset": {"name": "imagenet", "path": "/datasets/imagenet2"}}
        verify_dir_outputs(task.job_ret, task.overrides)
...
def test_app_with_config_groups__override_dataset__wrong(
    ...
    with (
        raises(MissingConfigException) as ex,
        hydra_task_runner(
            calling_file=calling_file,
            calling_module=calling_module,
            config_path="conf",
...
def test_app_with_config_groups__override_all_configs(
    ...
    with hydra_task_runner(
        ...
        assert task.job_ret is not None and task.job_ret.cfg == {"optimizer": {"type": "adam", "lr": 10, "beta": 0.01}}
...
def test_app_with_sweep_cfg__override_to_basic_launcher(
    ...
    with hydra_task_runner(
        ...
        assert hydra_cfg.hydra.launcher._target_ == "lerna._internal.core_plugins.basic_launcher.BasicLauncher"
...
def test_short_module_name(tmpdir: Path) -> None:
    ...
    assert OmegaConf.create(out) == {"db": {"driver": "mysql", "password": "secret", "user": "omry"}}
...
def test_hydra_main_module_override_name(tmpdir: Path) -> None:
    ...
    integration_test(
        ...
        env_override={"HYDRA_MAIN_MODULE": "lerna.test_utils.configs.Foo"},
    )


@mark.parametrize("env_name", ["HYDRA_MAIN_MODULE", "FB_PAR_MAIN_MODULE", "FB_XAR_MAIN_MODULE"])
def test_module_env_override(tmpdir: Path, env_name: str) -> None:
    ...
    modified_env[env_name] = "lerna.test_utils.configs.Foo"
...
def test_cfg_with_package(tmpdir: Path, flags: list[str], resolve: bool, expected: str) -> None:
    cmd = [
        "examples/tutorials/basic/your_first_hydra_app/5_defaults/my_app.py",
        f'hydra.run.dir="{normalize_path_for_override(tmpdir)}"',
        "hydra.job.chdir=True",
...
def test_cfg_resolve_interpolation(tmpdir: Path, script: str, resolve: bool, flags: list[str], expected: str) -> None:
    cmd = [
        script,
        f'hydra.run.dir="{normalize_path_for_override(tmpdir)}"',
        "hydra.job.chdir=True",
...
def test_help(tmpdir: Path, script: str, flags: list[str], overrides: list[str], expected: Any) -> None:
    cmd = [script, f'hydra.run.dir="{normalize_path_for_override(tmpdir)}"', "hydra.job.chdir=True"]
...
def test_searchpath_config(tmpdir: Path, overrides: list[str], expected: str) -> None:
    ...
    cmd.extend(overrides)
    cmd.extend([f'hydra.run.dir="{normalize_path_for_override(tmpdir)}"', "hydra.job.chdir=True"])
    result, _err = run_python_script(cmd)
...
def test_sys_exit(tmpdir: Path) -> None:
    ...
    assert subprocess.run(cmd, check=False).returncode == 42
...
def test_local_run_workdir(tmpdir: Path, task_config: DictConfig, overrides: list[str], expected_dir: str) -> None:
...
def test_hydra_env_set_with_config_interpolation(tmpdir: Path) -> None:
    cfg = OmegaConf.create({"foo": "bar", "hydra": {"job": {"env_set": {"foo": "${foo}"}}}})
    integration_test(
        tmpdir=tmpdir,
        task_config=cfg,
        overrides=[],
...
@mark.parametrize(
    "calling_file, calling_module",
    [
        param("lerna/test_utils/example_app.py", None, id="file"),
        param(None, "lerna.test_utils.example_app", id="module"),
    ],
)
def test_override_with_invalid_group_choice(
    ...
    with (
        raises(MissingConfigException) as e,
        hydra_task_runner(
            calling_file=calling_file,
            calling_module=calling_module,
            config_path="configs",
...
def test_module_run(tmpdir: Any, directory: str, file: str, module: str, error: str | None) -> None:
    cmd = [
        directory + "/" + file,
        f'hydra.run.dir="{normalize_path_for_override(tmpdir)}"',
        "hydra.job.chdir=True",
...
@mark.parametrize(
    "overrides,expected",
    [
        param(["dataset=imagenet"], {"dataset": {"name": "imagenet"}}, id="no_conf_dir"),
        param(
...
def test_defaults_pkg_with_dot(monkeypatch: Any, tmpdir: Path) -> None:
    ...
    result, _err = run_python_script(cmd)
    assert OmegaConf.create(result) == {"dataset": {"test": {"name": "imagenet", "path": "/datasets/imagenet"}}}
    ...
    [
        ...
        param(
            ...
            dedent(
                ...
                lerna\.errors\.HydraDeprecationError: Feature FooBar is deprecated
...
def test_hydra_deprecation_warning(env_deprecation_err: bool, expected: str, tmpdir: Path) -> None:
    ...
    _, err = run_python_script(cmd, env=env, allow_warnings=True, print_error=False, raise_exception=False)
