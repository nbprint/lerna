...
@mark.parametrize(
    ...
    [
        ...
        (
            ...
            OmegaConf.create({"db": {"driver": "mysql", "user": "omry", "password": "secret"}}),
        ),
    ],
)
...
@mark.parametrize(
    ...
    [
        (
            [],
            OmegaConf.create({"db": {"driver": "mysql", "password": "secret", "user": "omry"}}),
        )
    ],
)
...
def test_composition_config_example(hydra_restore_singletons: Any, hydra_task_runner: TTaskRunner) -> None:
...
def test_sweeping_example(hydra_restore_singletons: Any, hydra_sweep_runner: TSweepRunner) -> None:
