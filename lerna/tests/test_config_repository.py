...
@mark.parametrize(
    ...
    [
        param(
            FileConfigSource,
            "file://lerna/tests/test_apps/config_source_test/dir",
            id="FileConfigSource",
        ),
        param(
            ImportlibResourcesConfigSource,
            "pkg://lerna.tests.test_apps.config_source_test.dir",
...
class TestConfigRepository:
    def test_config_repository_load(self, hydra_restore_singletons: Any, path: str) -> None:
    ...
    def test_config_repository_exists(self, hydra_restore_singletons: Any, path: str) -> None:
        Plugins.instance()  # initializes
        config_search_path = create_config_search_path(path)
        repo = ConfigRepository(config_search_path=config_search_path)
...
@mark.parametrize(
    ...
    [
        ...
        (
            "#@package",
            raises(ValueError, match=re.escape("Expected header format: KEY VALUE, got")),
        ),
        (
            """# @package{sep}foo.bar
foo: bar
...
def test_importlib_resource_load_zip_path() -> None:
    ...
    conf = config_source._read_config(zipfile.Path("lerna/test_utils/configs/conf.zip", "config.yaml"))
