...
from lerna._internal.utils import run_and_report
from lerna.conf import HydraConf, RuntimeConf
from lerna.core.hydra_config import HydraConfig
from lerna.core.override_parser.overrides_parser import OverridesParser
from lerna.errors import HydraDeprecationError
...
def test_to_absolute_path(hydra_restore_singletons: Any, orig_cwd: str, path: str, expected: str) -> None:
...
def test_to_absolute_path_without_hydra(hydra_restore_singletons: Any, path: str, expected: str) -> None:
    # normalize paths to current OS
    path = str(Path(path))
...
def test_to_hydra_override_value_str_roundtrip(hydra_restore_singletons: Any, obj: Any) -> None:
...
def test_deprecation_warning(monkeypatch: Any, env_setting: str | None, expected_error: bool) -> None:
...
class TestRunAndReport:
    ...
    def test_simplified_traceback_with_no_module(self) -> None:
        ...
        with (
            ...
            raises(SystemExit, match="1"),
            patch("sys.stderr", new=mock_stderr),
            patch("inspect.getmodule", new=lambda *args: None),
        ):
            # Simulate a frame from a Python extension module or multithreaded code
            # where the Python module cannot be identified.
            run_and_report(demo_func)
        mock_stderr.seek(0)
        stderr_output = mock_stderr.read()
    ...
    def test_simplified_traceback_failure(self) -> None:
        ...
        with (
            ...
            patch("sys.stderr", new=mock_stderr),
            patch("traceback.print_exception", new=throws),
        ):
            run_and_report(demo_func)
        mock_stderr.seek(0)
        stderr_output = mock_stderr.read()
