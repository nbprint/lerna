...
from lerna.core.config_store import ConfigStore, ConfigStoreWithProvider
from lerna.core.override_parser.overrides_parser import OverridesParser
from lerna.core.utils import env_override, setup_globals
...
@mark.parametrize(
    "path",
    [
        param("file://lerna/test_utils/configs", id="file"),
        param("pkg://lerna.test_utils.configs", id="pkg"),
    ],
)
class TestConfigLoader:
    ...
    def test_load_with_missing_default(self, path: str) -> None:
        ...
        with raises(MissingConfigException):
            config_loader.load_configuration(config_name="missing-default.yaml", overrides=[], run_mode=RunMode.RUN)

    def test_load_with_optional_default(self, path: str) -> None:
        config_loader = ConfigLoaderImpl(config_search_path=create_config_search_path(path))
        cfg = config_loader.load_configuration(config_name="optional-default.yaml", overrides=[], run_mode=RunMode.RUN)
        with open_dict(cfg):
            del cfg["hydra"]
        assert cfg == {"foo": 10}
    ...
    def test_load_config_with_schema(self, hydra_restore_singletons: Any, path: str) -> None:
        ConfigStore.instance().store(name="config_with_schema", node=TopLevelConfig, provider="this_test")
        ConfigStore.instance().store(group="db", name="base_mysql", node=MySQLConfig, provider="this_test")
        ...
        with raises(HydraException):
            config_loader.load_configuration(config_name="db/mysql", overrides=["db.port=fail"], run_mode=RunMode.RUN)

    def test_load_config_file_with_schema_validation(self, hydra_restore_singletons: Any, path: str) -> None:
        ...
        msg = r"""'(config|db/mysql)' is validated against ConfigStore schema with the same name\.""" + re.escape(
            dedent(
                """
    ...
    def test_load_config_with_validation_error(self, hydra_restore_singletons: Any, path: str) -> None:
        ConfigStore.instance().store(name="base_mysql", node=MySQLConfig, provider="this_test")
        config_loader = ConfigLoaderImpl(config_search_path=create_config_search_path(path))
    ...
    def test_load_config_with_key_error(self, hydra_restore_singletons: Any, path: str) -> None:
        ConfigStore.instance().store(name="base_mysql", node=MySQLConfig, provider="this_test")
        config_loader = ConfigLoaderImpl(config_search_path=create_config_search_path(path))
    ...
    def test_assign_null(self, hydra_restore_singletons: Any, path: str) -> None:
        config_loader = ConfigLoaderImpl(config_search_path=create_config_search_path(path))
        cfg = config_loader.load_configuration(config_name="config.yaml", overrides=["+abc=null"], run_mode=RunMode.RUN)
        with open_dict(cfg):
            del cfg["hydra"]
        assert cfg == {"normal_yaml_config": True, "abc": None}

    def test_sweep_config_cache(self, hydra_restore_singletons: Any, path: str, monkeypatch: Any) -> None:
        setup_globals()

        # pseudorandom resolvers with/without value caching
        pseudorandom_values = iter(["1st", "2nd", "3rd", "4th"])
        OmegaConf.register_new_resolver("cached", lambda: next(pseudorandom_values), use_cache=True, replace=True)
        OmegaConf.register_new_resolver("uncached", lambda: next(pseudorandom_values), use_cache=False, replace=True)
        ...
        assert isinstance(master_cfg.time, str)
        assert isinstance(master_cfg.test_env, str)
        assert isinstance(master_cfg.test_cached, str)  # "1st"
...
def test_defaults_not_list_exception() -> None:
    config_loader = ConfigLoaderImpl(config_search_path=create_config_search_path("lerna/test_utils/configs"))
...
def test_load_schema_as_config(hydra_restore_singletons: Any) -> None:
    ...
    """
    ConfigStore.instance().store(name="config", node=TopLevelConfig, provider="this_test")

    ConfigStore.instance().store(name="db/mysql", node=MySQLConfig, provider="this_test")

    config_loader = ConfigLoaderImpl(config_search_path=create_config_search_path(None))
    cfg = config_loader.load_configuration(config_name="config", overrides=[], run_mode=RunMode.RUN)
...
def test_overlapping_schemas(hydra_restore_singletons: Any) -> None:
    ...
    cfg = config_loader.load_configuration(config_name="config", overrides=["+plugin=concrete"], run_mode=RunMode.RUN)
    with open_dict(cfg):
        del cfg["hydra"]
...
def test_invalid_plugin_merge(hydra_restore_singletons: Any) -> Any:
    ...
    with raises(HydraException):
        cl.load_configuration(config_name="config", overrides=["plugin=invalid"], run_mode=RunMode.RUN)
...
@mark.parametrize(
    ...
    [
        ...
        param(
            ...
            raises(
                HydraException,
                match=re.escape("Could not override 'x'.\nTo append to your config use +x=10"),
            ),
        ...
        param(
            ...
            raises(
                HydraException,
                match=re.escape("Could not append to config. An item is already at 'x'"),
            ),
        ...
        param(
            ...
            raises(
                HydraException,
                match=re.escape("Could not delete from config. The value of 'x' is 20 and not 10."),
            ),
        ...
        param(
            ...
            raises(
                HydraException,
                match=re.escape("Override foo@bar=10 looks like a config group override, but config group 'foo' does not exist."),
            ),
