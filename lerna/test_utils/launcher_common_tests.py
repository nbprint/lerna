...
def sweep_1_job(
    ...
    with sweep:
        ...
        assert job_ret[0].hydra_cfg.hydra.job.name == "a_module", "Unexpected job name: " + job_ret[0].hydra_cfg.hydra.job.name
...
def sweep_2_jobs(
    ...
    with sweep:
        ...
        for i in range(2):
            ...
            assert job_ret.hydra_cfg.hydra.job.name == "a_module", "Unexpected job name: " + job_ret.hydra_cfg.hydra.job.name
...
class IntegrationTestSuite:
    ...
    def test_custom_sweeper_run_workdir(
        ...
        expected_outputs = (
            str(self.get_test_scratch_dir(test_app_dir) / expected_dir) if test_app_dir else str(self.get_test_scratch_dir(tmpdir) / expected_dir)
        )
    ...
    def test_get_orig_dir_multirun(self, tmpdir: Path, task_launcher_cfg: DictConfig, extra_flags: List[str]) -> None:
        ...
        integration_test(
            ...
            task_config=cfg,
            overrides=overrides,
            prints="lerna.utils.get_original_cwd()",
    ...
    def test_to_absolute_path_multirun(self, tmpdir: Path, task_launcher_cfg: DictConfig, extra_flags: List[str]) -> None:
        ...
        integration_test(
            ...
            task_config=cfg,
            overrides=overrides,
            prints="lerna.utils.to_absolute_path('/foo/bar')",
        ...
        integration_test(
            ...
            task_config=cfg,
            overrides=overrides,
            prints=["lerna.utils.to_absolute_path('foo/bar')", "os.getcwd()"],
