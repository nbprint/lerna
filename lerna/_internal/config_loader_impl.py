...
from lerna._internal.config_repository import (
    CachingConfigRepository,
    ConfigRepository,
    IConfigRepository,
)
from lerna._internal.defaults_list import DefaultsList, create_defaults_list
from lerna.conf import ConfigSourceInfo
...
# Try to import Rust bindings for accelerated compose
try:
    import lerna.lerna as _rs
    from lerna._internal.core_plugins import pkg_helper, structured_helper

    _HAS_RUST = True
    _HAS_HYBRID_REPO = hasattr(_rs, "RustHybridConfigRepository")
except ImportError:
    _HAS_RUST = False
    _HAS_HYBRID_REPO = False
...
class ConfigLoaderImpl(ConfigLoader):
    ...
    def _load_configuration_impl(
        ...
        # Convert Version object to string for OmegaConf compatibility
        if version_base is not None and not isinstance(version_base, str):
            version_base = str(version_base)
        cfg.hydra.runtime.version_base = version_base
        ...
        for override in overrides:
            if override.package is not None:
                raise ConfigCompositionException(
                    f"Override {override.input_line} looks like a config group override, but config group '{override.key_or_group}' does not exist."
                )
    ...
    def _try_rust_compose(
        ...
        This is the primary code path. Returns None only if Rust bindings
        are not available, otherwise raises on errors.

        Returns None to fall back to Python when:
        - Rust bindings are not available
        - Structured configs with schema validation are detected
        """
        if not _HAS_RUST:
            return None

        if not _HAS_HYBRID_REPO:
            return None

        # Check if any defaults come from structured source (ConfigStore)
        # Structured configs need Python path for proper schema validation
        for default in defaults:
            if default.config_path is not None:
                # Check if this config exists in ConfigStore
                ...
                if not path.endswith(".yaml"):
                    path = f"{path}.yaml"
                # Use _open to check if path exists in ConfigStore
                if cs._open(path) is not None:
                    # Fall back to Python for proper OmegaConf merge with schema
                    return None

        # Check source types
        sources = repo.get_sources()
        has_pkg_sources = False
        has_structured_sources = False
        for src in sources:
            scheme = src.scheme()
            if scheme == "pkg":
                has_pkg_sources = True
            if scheme == "structured":
                has_structured_sources = True

        # Build source tuples for Rust repo
        source_tuples = []
        for src in sources:
            scheme = src.scheme()
            path = src.path
            if scheme == "file":
                source_tuples.append((src.provider, f"file://{path}"))
            elif scheme == "pkg":
                source_tuples.append((src.provider, f"pkg://{path}"))
            elif scheme == "structured":
                source_tuples.append((src.provider, "structured://"))

        if not source_tuples:
            raise ConfigCompositionException("No config sources available")

        # Create Rust hybrid repo with all callbacks
        rust_repo = _rs.RustHybridConfigRepository(
            source_tuples,
            pkg_loader=pkg_helper.load_pkg_config if has_pkg_sources else None,
            pkg_config_exists=pkg_helper.pkg_config_exists if has_pkg_sources else None,
            pkg_group_exists=pkg_helper.pkg_group_exists if has_pkg_sources else None,
            pkg_list_options=pkg_helper.pkg_list_options if has_pkg_sources else None,
            structured_loader=structured_helper.load_structured_config if has_structured_sources else None,
            structured_config_exists=structured_helper.structured_config_exists if has_structured_sources else None,
            structured_group_exists=structured_helper.structured_group_exists if has_structured_sources else None,
            structured_list_options=structured_helper.structured_list_options if has_structured_sources else None,
        )

        # Compose each default using Rust repo's load_config
        result: Any = {}
        for default in defaults:
            if default.config_path is None:
                continue

            loaded = rust_repo.load_config(default.config_path)
            if loaded is None:
                raise MissingConfigException(
                    missing_cfg_file=default.config_path,
                    message=f"Config not found: {default.config_path}",
                    options=None,
                )

            # Handle string configs like "abc=cde" which OmegaConf treats as {key: None}
            # This matches OmegaConf.create() behavior where a bare string becomes a dict key
            if isinstance(loaded, str):
                loaded = {loaded: None}

            # Remove defaults key if it's a dict
            if isinstance(loaded, dict) and "defaults" in loaded:
                del loaded["defaults"]

            # Merge at package path
            package = default.package
            if package is None or package == "" or package == "_global_":
                # Direct merge or replace
                if isinstance(loaded, dict) and isinstance(result, dict):
                    self._deep_merge_dicts(result, loaded)
                else:
                    # Non-dict configs (lists) replace the result
                    result = loaded
            else:
                # Create nested structure for package
                ...
                for part in parts[:-1]:
                    ...
                    nested = nested[part]
                # Set the final key
                if isinstance(loaded, dict) and parts[-1] in nested and isinstance(nested[parts[-1]], dict):
                    self._deep_merge_dicts(nested[parts[-1]], loaded)
                else:
                    nested[parts[-1]] = loaded
    ...
    def _deep_merge_dicts(target: dict, source: dict) -> None:
        ...
        if not isinstance(source, dict):
            # If source is not a dict, we can't merge - just return
            return
        for key, value in source.items():
            if key in target and isinstance(target[key], dict) and isinstance(value, dict):
                ConfigLoaderImpl._deep_merge_dicts(target[key], value)
            else:
                target[key] = value

    @staticmethod
    def _strip_defaults(cfg: Any) -> None:
        """Remove defaults lists from all nodes in a config."""
        if isinstance(cfg, DictConfig):
            ...
            for _key, value in cfg.items_ex(resolve=False):
                ConfigLoaderImpl._strip_defaults(value)
