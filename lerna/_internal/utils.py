...
from lerna.core.config_search_path import ConfigSearchPath, SearchPathQuery
from lerna.core.utils import get_valid_filename, validate_config_path
from lerna.errors import (
    CompactHydraException,
    InstantiationException,
    SearchPathException,
)
from lerna.types import RunMode, TaskFunction
...
def create_config_search_path(search_path_dir: Optional[str]) -> ConfigSearchPath:
    ...
    search_path.append("lerna", "pkg://lerna.conf")
...
def _run_hydra(
    ...
    search_path = create_automatic_config_search_path(calling_file, calling_module, config_path)

    def add_conf_dir() -> None:
        if args.config_dir is not None:
            abs_config_dir = os.path.abspath(args.config_dir)
            if not os.path.isdir(abs_config_dir):
                raise SearchPathException(f"Additional config directory '{abs_config_dir}' not found")
            search_path.prepend(
    ...
    hydra = run_and_report(lambda: Hydra.create_main_hydra2(task_name=task_name, config_search_path=search_path))
    ...
    try:
        ...
        if args.hydra_help:
            hydra.hydra_help(config_name=config_name, args_parser=args_parser, args=args)
            sys.exit(0)
        ...
        if args.run or args.multirun:
            ...
            run_and_report(lambda: hydra.shell_completion(config_name=config_name, overrides=args.overrides))
        elif args.info:
            hydra.show_info(args.info, config_name=config_name, overrides=args.overrides)
        else:
