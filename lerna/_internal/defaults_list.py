...
def _validate_self(
    ...
    for d in defaults:
        ...
        if d.is_self():
            if has_self:
                raise ConfigCompositionException(f"Duplicate _self_ defined in {containing_node.get_config_path()}")
            has_self = True
...
def _create_defaults_tree_impl(
    ...
    self_added = False
    if len(defaults_list) > 0 or is_root_config and len(overrides.append_group_defaults) > 0:
        has_config_content = isinstance(loaded.config, DictConfig) and _has_config_content(loaded.config)
        ...
        if d.is_self():
            ...
            if isinstance(d, GroupDefault) and d.is_options():
                ...
                for item in reversed(d.get_options()):
                    if "${" in item:
                        raise ConfigCompositionException(f"In '{path}': Defaults List interpolation is not supported in options list items")
                    ...
                    node.update_parent(parent.get_group_path(), parent.get_final_package())
...
def ensure_no_duplicates_in_list(result: List[ResultDefault]) -> None:
    ...
    for item in result:
        if not item.is_self:
            ...
            if key is not None:
                if key in keys:
                    raise ConfigCompositionException(f"{key} appears more than once in the final defaults list")
                keys.add(key)
...
def create_defaults_list(
    ...
    :return:
    """
    # Use Python implementation for defaults list
    # Config composition is accelerated by Rust in _try_rust_compose()
...
def create_defaults_list_rust(
    ...
    Returns None - this function is not currently used in production.
    The RustHybridConfigRepository (needed for pkg:// and structured://)
    does not have load_and_compose. Config composition is done separately
    via _try_rust_compose() in config_loader_impl.py.

    This function is kept for future use if load_and_compose is added
    to RustHybridConfigRepository.
    """
    return None
