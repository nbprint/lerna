...
def _is_blocklisted_target(target: str) -> bool:
    ...
    if canonical_target in DEFAULT_BLOCKLISTED_MODULES or canonical_target in UNCONTROLLED_EXECUTION_TARGETS:
        return True
...
def _get_resolved_target_name_for_check(target: Callable[..., Any]) -> str:
    ...
    if getattr(target, "__name__", None) == "__get__":
        descriptor_binding_target = _CALLABLE_DESCRIPTOR_BINDING_TARGETS.get(descriptor_owner) if isinstance(descriptor_owner, type) else None
...
def _authorize_target_name(
    ...
    *,
    ...
    if target_name in allowlist_entries or canonical_target in allowlist_entries or (resolved_from_is_alias and resolved_from in allowlist_entries):
        return
    msg = dedent(
        f"""\
...
def _authorize_target_invocation(
    ...
    *,
    ...
    if target_name in _NON_CALLABLE_MOCK_TARGETS:
        unsafe_parameters = sorted(set(kwargs).difference(_NON_CALLABLE_MOCK_SAFE_PARAMETERS))
