...
class FloatRange:
    ...
    def __next__(self) -> float:
        ...
        if self.step > 0:
            ...
            raise HydraException(f"Invalid range values (start:{self.start}, stop:{self.stop}, step:{self.step})")
...
class RangeSweep(Sweep):
    ...
    def range(self) -> Union[range, FloatRange]:
        ...
        stop = self.stop
        step = self.step
        if isinstance(start, int) and isinstance(stop, int) and (step is None or isinstance(step, int)):
...
class Glob:
    ...
    def filter(self, names: List[str]) -> List[str]:
        ...
        if _HAS_RUST:
            ...
            rust_glob = _rs.Glob(self.include, self.exclude)
...
class Override:
    ...
    def sweep_iterator(self, transformer: TransformerType = Transformer.identity) -> Iterator[ElementType]:
        """
        ...
        if self.value_type not in (
            ...
            raise HydraException(f"Can only enumerate CHOICE and RANGE sweeps, type is {self.value_type}")
    ...
    @staticmethod
    def _get_value_element_as_str(value: ParsedElementType, space_after_sep: bool = False) -> str:
        ...
        if value is None:
            ...
            s = comma.join([Override._get_value_element_as_str(x, space_after_sep=space_after_sep) for x in value])
            return "[" + s + "]"
        elif isinstance(value, dict):
            str_items = []
            for k, v in value.items():
                str_key = Override._get_value_element_as_str(k)
                str_value = Override._get_value_element_as_str(v, space_after_sep=space_after_sep)
            ...
            return Override._get_value_element_as_str(OmegaConf.to_container(OmegaConf.structured(value)))
        else:
            assert False
